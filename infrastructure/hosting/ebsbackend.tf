...
resource "aws_elastic_beanstalk_application" "dev_teams_app" {
  name        = "dev-teams-app"
  description = "Dev Teams App"
  tags        = var.common_tags
}

resource "aws_elastic_beanstalk_environment" "dev_teams_app_env" {
  name                = "dev-teams-app-env"
  application         = aws_elastic_beanstalk_application.dev_teams_app.name
  solution_stack_name = "64bit Amazon Linux 2023 v6.1.3 running Node.js 20"
  cname_prefix        = "dev-teams"
  tags                = var.common_tags
